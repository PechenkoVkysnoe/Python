...
from .models import Category, Short, LongShort, Dress, CartProduct, Cart, Customer
from .views import CategoryDetailView, CheckoutView, recalc_cart, AddToCartView, BaseView, DeleteFromCartView
...
@pytest.fixture
def category(db):
    category = Category.objects.create(name='Рубаха', slug='longshorts')
    return category


@pytest.fixture
def longshort(db, get_image_file1, category):
    image = get_image_file1
    longshort = LongShort.objects.create(
        category=category,
        title="Test pizza",
        slug="test-slug",
        image=image,
